<title>5 Strings</title> <link href="../../page_styles.css" rel="stylesheet" type="text/css"> <link href="../../stylesheet.css" rel="stylesheet" type="text/css">
...
如果不讨论围棋中的符文概念，我们就不能开始这一章。正如您将在接下来的部分中看到的，这个概念是彻底理解如何处理字符串和避免常见错误的关键。但是在深入研究 Go runes 之前，我们需要确保我们在一些基本的编程概念上是一致的。
...
同时，我们提到 UTF-8 将字符编码成 1 到 4 个字节，因此，最多 32 位。这就是为什么在围棋中，符文是`int32`的别名:

```
type rune = int32
```

关于 UTF-8，另一件要强调的事情是:有些人认为围棋弦总是 UTF-8，但这不是真的。让我们考虑下面的例子:

```
s := "hello"
```

我们将一个字符串文字(一个字符串常量)赋给`s`。在围棋中，源代码是用 UTF 8 编码的。因此，所有的字符串都使用 UTF-8 编码成一个字节序列。然而，字符串是任意字节的序列；它不一定基于 UTF-8。因此，当我们操作一个不是从字符串初始化的变量时(例如，从文件系统中读取)，我们不能假定它使用 UTF-8 编码。
...
记住这些概念是必要的，因为在围棋中，符文无处不在。让我们来看看这一知识的具体应用，它涉及一个与字符串迭代相关的常见错误。
...
![](../../OEBPS/Images/CH05_F01_Harsanyi.png)
...
围棋开发者在使用`strings`包时的一个常见错误是将的`TrimRight`和`TrimSuffix`混在一起。这两个功能的目的相似，很容易混淆。让我们来看看。
...
![](../../OEBPS/Images/CH05_F02_Harsanyi.png)
...
运行这段代码会打印出`abc`，而不是`axc`。的确，在围棋中，一个字符串是不可变的。
