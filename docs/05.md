<title>6 Functions and methods</title> <link href="../../page_styles.css" rel="stylesheet" type="text/css"> <link href="../../stylesheet.css" rel="stylesheet" type="text/css">
...
还要注意，在某些上下文中，已经初始化的结果参数可能非常方便，即使它们不一定有助于可读性。下面这个例子提出的中的*有效 Go* ( [`go.dev/doc/effective_go`](https://go.dev/doc/effective_go) )是受中`io.ReadFull`功能的启发:
...
在本节中，我们将讨论返回接口的影响，以及为什么在某些情况下这样做会导致错误。这个错误可能是围棋中最普遍的错误之一，因为它可能被认为是违反直觉的，至少在我们犯这个错误之前是这样。
...
在围棋中，我们要知道一个指针接收器可以是`nil`。让我们通过创建一个虚拟类型并调用一个具有 nil 指针接收器的方法来进行实验:
...
![](../../OEBPS/Images/CH06_F01_Harsanyi.png)
...
克服这些限制的一个方法可能是让函数接受一个`*bufio.Scanner`(由`bufio.NewScanner`返回的输出)。从我们创建`scanner`变量的那一刻起，这两个函数就有相同的逻辑，所以这种方法是可行的。但在围棋中，惯用的方式是从读者的抽象出发。
