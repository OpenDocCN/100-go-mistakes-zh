...
HTTP 处理程序接受两个参数:请求和编写响应的方式。`httptest`包为两者提供了实用工具。对于请求，我们可以使用 HTTP 方法、URL 和主体使用`httptest.NewRequest`构建一个`*http.Request`。对于响应，我们可以使用`httptest.NewRecorder`来记录处理程序中的变化。让我们编写这个处理程序的单元测试:
