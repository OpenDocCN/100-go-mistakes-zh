...
因此，运行带有特定标签的测试包括没有标签的文件和匹配这个标签的文件。如果我们只想运行*集成测试*呢？一种可能的方法是在单元测试文件上添加一个否定标记。例如，使用`!integration`意味着只有当`integration`标志*不是*启用时，我们才想要包含测试文件(contract_test.go):
